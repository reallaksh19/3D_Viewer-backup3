...
POINT_FIELDS = ("ComponentType", "DTXR_POS", "DTXR_PS", "PipingClass", "Rating", "Position")
...
def _add_restraint(parent: ET.Element, restraint: base.RestraintSpec, node_number: int) -> None:
    ET.SubElement(parent, "RESTRAINT", {
        ...
        "FRIC_COEF": _fmt(restraint.friction),
        "CNODE": "0",
        "XCOSINE": "-1.0101",
        "YCOSINE": "-1.0101",
        "ZCOSINE": "-1.0101",
...
def _inputxml_from_final_xml(final_xml: Path, support_config: dict[str, Any], use_json_restraints: bool) -> ET.ElementTree:
    ...
    explicit_specs: dict[int, tuple[base.RestraintSpec, ...]] = dtxr_specs if use_json_restraints else dict(xml_specs)
    if not use_json_restraints:
        for node_number, specs in dtxr_specs.items():
            explicit_specs[node_number] = patched._merge_specs(explicit_specs.get(node_number, tuple()), specs)
    ...
    for branch in document.branches:
        ...
        for from_node, to_node in zip(nodes, nodes[1:]):
            ...
            attrs = {
                ...
                "MATERIAL_NUM": _fmt(material), "FLUID_DENSITY": _fmt(branch.branch_fluid_density, 6), "HYDRO_PRESSURE": _fmt(hydro_by_branch.get(branch.branch_name, 0.0), 6),
            }
            ...
            if to_node.component_type.upper() in {"TEE", "OLET", "BRAN"}:
                ET.SubElement(element, "SIF", {"NODE": _fmt(to_node.node_number)})
            for slot, restraint in enumerate(explicit_specs.get(to_node.node_number, tuple()), start=1):
                _add_restraint(element, restraint, to_node.node_number)
