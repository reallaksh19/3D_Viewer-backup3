...
def _lookup_class_bore(pclass_master, cls, bore):
    """class + bore -> {wall, corr, mat_name, rating} from the piping class master."""
    if not (cls and pclass_master):
        return {}
    for r in pclass_master:
        rcls = str(mrow(r, "Piping Class", "pipingClass") or "")
        rbore = mrow(r, "convertedBore", "Converted Bore")
        if rbore is None:
            rbore = nps_to_dn(mrow(r, "Size", "size"))
        if MC.normalize(rcls) == MC.normalize(cls) and (
                bore is None or rbore is None or abs(float(rbore) - bore) < 1):
            return {
                "wall": mrow(r, "Wall thickness", "Wall Thickness", "wallThickness"),
                "corr": mrow(r, "Corrosion", "corrosionAllowance"),
                "mat_name": mrow(r, "Material_Name", "Material", "material"),
                "rating": mrow(r, "Rating", "rating"),
            }
    return {}


def _resolve_class(bname, line_key, ll_index, known_classes, overrides, cfg):
    """Piping class: line list (by derived key) wins; else a branch token that
    approximately matches a known class (rejects spec/job codes like 91261M7)."""
    ll_row = ll_index.get(MC.normalize(line_key)) if line_key else None
    if ll_row and mrow(ll_row, "pipingClass", "Piping Class"):
        return {"cls": str(mrow(ll_row, "pipingClass", "Piping Class")),
                "method": "linelist", "confidence": 1.0, "needs_review": False, "ll_row": ll_row}
    if known_classes:
        for tok in class_tokens(bname):
            cand = MC.approximate_class_match(tok, known_classes, overrides, cfg)
            if cand["pipingClass"]:
                return {"cls": cand["pipingClass"], "method": cand["method"],
                        "confidence": cand["confidence"], "needs_review": cand["needs_review"], "ll_row": ll_row}
    return {"cls": None, "method": "none", "confidence": 0.0, "needs_review": True, "ll_row": ll_row}
...
def _resolve_line(bname, bore, ctx):
    ...
    cr = _resolve_class(bname, line_key, ctx["ll_index"], ctx["known_classes"], ctx["overrides"], ctx["cfg"])
    cls = cr["cls"]
    rating = (mrow(cr["ll_row"], "rating", "Rating") if cr["ll_row"] else None)
    cb = _lookup_class_bore(ctx["pclass_master"], cls, bore)
    wall, corr, mat_name = cb.get("wall"), cb.get("corr"), cb.get("mat_name")
    if rating is None:
        rating = cb.get("rating")
    if mat_name:
        mat = MC.resolve_material_code(mat_name, ctx["material_map"], ctx["overrides"], ctx["cfg"])
    else:
        mat = {"code": None, "name": mat_name, "method": "none", "confidence": 0.0, "needs_review": False}
    return {
        "branch": bname, "lineNoKey": line_key, "bore_mm": bore,
        "pipingClass": cls, "class_method": cr["method"], "class_confidence": round(cr["confidence"], 3),
        "rating": rating, "wallThickness": wall, "corrosion": corr,
        "materialName": mat_name, "materialCode": mat["code"],
        "material_method": mat["method"], "material_confidence": round(mat["confidence"], 3),
        "needs_review": bool(cr["needs_review"] or mat["needs_review"]),
    }
