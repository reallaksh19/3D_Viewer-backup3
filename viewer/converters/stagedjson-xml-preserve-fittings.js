...
function installPanel() {
  ...
  const converterSelect = root.querySelector('#model-converters-select');
  const updateVisibility = () => {
    const selectedConverter = converterSelect ? String(converterSelect.value || '') : '';
    panel.style.display = selectedConverter === 'stagedjson_to_xml' ? '' : 'none';
  };
  if (converterSelect && !converterSelect.dataset.sjxmlPreserveBound) {
    converterSelect.dataset.sjxmlPreserveBound = '1';
    converterSelect.addEventListener('change', updateVisibility);
  }
  updateVisibility();

  const status = panel.querySelector('[data-sjxml-status]');
  const runButton = panel.querySelector('[data-sjxml-run]');
  if (runButton && !runButton.dataset.sjxmlPreserveBound) {
    runButton.dataset.sjxmlPreserveBound = '1';
