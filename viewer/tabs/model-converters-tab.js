...
const XML_CII_WORKFLOW_PHASES = Object.freeze([
  ...
  { id: 'preview',        label: '3 Preview',        summary: 'Dry-run enrichment preview per branch \u2014 inspect and override approximate matches.', state: 'Current' },
  { id: 'diagnostics',    label: '4 Diagnostics',    summary: 'Run Dry Run to inspect enrichment data before committing to the full run.', state: 'Current' },
  { id: 'weight-match',   label: '4A Weight Match',  summary: 'Review approximate component weights matched by bore, rating, and length, then Finalize and Run.', state: 'Current' },
  { id: 'run',            label: '5 Run',            summary: 'Run the conversion \u2014 generates enriched XML and final CII output.', state: 'Current' },
  { id: 'support-mapper', label: '6 Support Types',  summary: 'Map ATT/RVM fields to CII support kinds for enrichment and 3D symbol rendering.', state: 'Current' },
  { id: 'config',         label: '7 Config',         summary: 'Edit all enrichment configuration fields and export/import the JSON config.', state: 'Current' },
]);
...
const XML_CII_MASTER_DEFS = Object.freeze({
  ...
  pipingClass: Object.freeze({
    ...
    rowsKey: 'masterRows',
    fieldMapKey: 'fieldMap',
    defaultUrl: '../docs/Masters/Piping_class_master.json',
  ...
  material: Object.freeze({
    ...
    rowsKey: 'mapRows',
    fieldMapKey: 'fieldMap',
    defaultUrl: '../docs/Masters/PCF_MAT_MAP.TXT',
    requiredFields: Object.freeze([]),
    aliases: Object.freeze({
  ...
  weight: Object.freeze({
    ...
    rowsKey: 'masterRows',
    fieldMapKey: 'fieldMap',
    defaultUrl: '../docs/Masters/wtValveweights.json',
  ...
  xml_to_cii: {
    ...
    defaults: {
      ...
      supportConfigJson: JSON.stringify({
        ...
        pipingClass: {
          masterUrl: '../docs/Masters/Piping_class_master.json',
          masterRows: [],
...
function _xmlCiiMapRowsWithFieldMap(rawRows, fieldMap) {
...
function _xmlCiiRowsToTableHtml(rows, maxRows) {
  ...
  const headers = _xmlCiiHeadersFromRows(safeRows).filter((header) => header !== '_raw').slice(0, 18);
  return `
...
// Imported master tables (line list / piping class / material map / valve
// weights) can be several MB and overflow the ~5MB localStorage quota. These
// paths point at the bulky arrays inside an XML->CII supportConfigJson so they
// can be dropped from the persisted copy when storage is full.
...
// Return a localStorage-safe copy of the converter defaults with the heavy
// master arrays emptied. Masters reload from their source URLs (or can be
// re-imported), so dropping them from storage keeps field maps, regex, run
// options, and manual overrides while staying under quota. The in-memory state
// is never mutated.
...
// Persist state to localStorage. Never throws: on a quota error (large imported
// masters) it retries with a storage-safe copy. Returns 'full' | 'trimmed' |
// 'failed' so callers can surface a one-time notice.
...
function _parseXmlCiiEnrichmentConfig(rawJson) {
  const defaults = {
    ...
    // Piping class master lookup: class + bore -> wall thickness, corrosion,
    // material name, rating. Approximate (exact -> startsWith -> fuzzy) so an
    // imperfect class still resolves and is flagged needs_review.
    pipingClass: {
      masterUrl: '../docs/Masters/Piping_class_master.json',
      masterRows: [],
      startsWithConfidence: 0.8,
    ...
    // Material name -> code via a Material Map (exact -> contains -> token Jaccard).
    material: {
      masterUrl: '../docs/Masters/PCF_MAT_MAP.TXT',
      mapRows: [],
      containsConfidence: 0.9,
    ...
    // Manual overrides (highest precedence): class/material/process data and rigid weights.
    overrides: { pipingClass: {}, material: {}, rigidWeight: {} },
  };
  const text = _toText(rawJson).trim();
  ...
  try {
    ...
    const supportKindToXmlType = _normalizeXmlCiiSupportKindToTypeConfig({
      ...defaults.supportKindToXmlType,
      ...(parsed.supportKindToXmlType || {}),
    });
    return {
      ...defaults,
      ...parsed,
      ...
      weight: { ...defaults.weight, ...(parsed.weight || {}) },
      pipingClass: { ...defaults.pipingClass, ...(parsed.pipingClass || {}) },
      material: { ...defaults.material, ...(parsed.material || {}) },
      overrides: {
        pipingClass:  { ...(defaults.overrides.pipingClass),  ...((parsed.overrides || {}).pipingClass  || {}) },
        material:     { ...(defaults.overrides.material),     ...((parsed.overrides || {}).material     || {}) },
        rigidWeight:  { ...(defaults.overrides.rigidWeight),  ...((parsed.overrides || {}).rigidWeight  || {}) },
        // processData: user-typed P1/T1/Density overrides keyed by lineKey — MUST be preserved
        processData:  { ...((parsed.overrides || {}).processData || {}) },
      },
    };
  } catch {
    return defaults;
...
function _buildStagedSupportIndex(stagedJsonText, config, diagnostics = []) {
  ...
  try {
    ...
    diagnostics.push({ type: 'staged-json-parse-error', message: _toText(error?.message || error) });
    return empty;
  }
  ...
  for (const entry of _walkStagedComponents(parsed)) {
    ...
    const kind = _resolveXmlCiiSupportKind(attrs);
    if (!kind) continue;
    let point = null;
    for (const key of ['SUPPORTCOORD', 'POS', 'BPOS', 'APOS', 'LPOS']) {
      point = _normalizePoint(attrs[key]);
      if (point) break;
    }
    const indexed = { ...entry, kind, point };
    if (point) {
...
function _xmlCiiDtxrPsForNode(node, stagedComponentIndex) {
  ...
  const tags = _xmlNodeSupportTags(node);
  const values = [];
  for (const tag of tags) {
    const matches = stagedComponentIndex.byTag.get(tag) || [];
    for (const match of matches) {
      const dtxr = _stagedComponentDtxr(match);
      if (dtxr && !values.includes(dtxr)) values.push(dtxr);
    }
  }
  return { text: values.join('|'), tags, values };
}

function _xmlCiiDtxrPositionOffset(config) {
  const option = config?.dtxrPositionOffset || {};
  return {
    enabled: option.enabled === true,
    xOffset: _toFiniteNumber(option.xOffset, 0),
    yOffset: _toFiniteNumber(option.yOffset, 0),
    zOffset: _toFiniteNumber(option.zOffset, 0),
    tolerance: Math.max(_toFiniteNumber(option.tolerance, 0.5), 0),
  };
}

function _xmlCiiApplyDtxrPositionOffset(point, config) {
  if (!point) return null;
  const option = _xmlCiiDtxrPositionOffset(config);
  if (!option.enabled) return point;
  return {
    x: point.x + option.xOffset,
    y: point.y + option.yOffset,
    z: point.z + option.zOffset,
  };
}

function _xmlCiiPositionsMatch(left, right, tolerance) {
...
function _buildStagedDtxrPositionIndex(stagedJsonText, config) {
  ...
  for (const entry of _walkStagedComponents(parsed)) {
    const attrs = entry.attrs || {};
    const dtxr = _stagedComponentDtxr(entry);
    const rawPoint = _normalizePoint(attrs.POSI);
    const point = _xmlCiiApplyDtxrPositionOffset(rawPoint, config);
    if (!dtxr || !point) continue;
    entries.push({ ...entry, dtxr, point, rawPoint });
  }
...
function _xmlCiiDtxrPosForNode(node, dtxrPositionIndex, config) {
  ...
  const target = _normalizePoint(_xmlText(node, 'Position'));
  if (!target) return { text: '', values: [] };
  const tolerance = _xmlCiiDtxrPositionOffset(config).tolerance;
  const values = [];
  for (const entry of dtxrPositionIndex.entries) {
    if (!_xmlCiiPositionsMatch(entry.point, target, tolerance)) continue;
...
function _deriveBranchLineKey(branchName, config) {
  ...
  const byToken = _xmlCiiLineKeyFromBranchTokens(text, config);
...
function _xmlCiiApplyLineListProcessData(document, branch, row, config, branchLineKey) {
  ...
  if (p1) {
    _xmlSetText(document, _xmlEnsureChild(document, branch, 'Pressure'), 'Pressure1', p1);
    count += 1;
  }
...
function _findWeightMasterMatch({ boreMm, rating, lengthMm }, config) {
  const best = _findAllWeightCandidates({ boreMm, rating, lengthMm }, config)[0] || null;
  return best || null;
}

function _findAllWeightCandidates({ boreMm, rating, lengthMm }, config) {
  const rows = Array.isArray(config.weight?.masterRows) ? config.weight.masterRows : [];
  if (!rows.length || boreMm === null || !rating || lengthMm === null) return [];
  const tolerance = _toFiniteNumber(config.weight?.lengthToleranceMm, 4);
  const normalizedRating = _toText(rating).replace(/#/g, '').toUpperCase();
  const candidates = [];
  for (const row of rows) {
    const rowBore = _rowNumber(row, ['boreMm', 'convertedBore', 'Converted Bore', 'bore', 'Bore', 'DN', 'NB']);
    const rowLengthRaw = _rowNumber(row, ['lengthMm', 'length', 'Length', 'Length (RF-F/F)', 'RF-F/F', 'LEN', 'faceToFace']);
    const unit = _toText(config.weight?.masterLengthUnit || 'auto').toLowerCase();
    const rowLength = rowLengthRaw === null
      ? null
      : (unit === 'in' || (unit === 'auto' && rowLengthRaw < 100 && lengthMm > 100)
        ? rowLengthRaw * _toFiniteNumber(config.weight?.inchToMm, 25.4)
        : rowLengthRaw);
    const rowWeight = _rowNumber(row, ['valveWeight', 'directWeight', 'weight', 'Weight', 'RF/RTJ KG', 'Valve Weight']);
    const rowRating = _rowText(row, ['ratingClass', 'rating', 'Rating', 'RATING', 'Class', 'CLASS', 'Pressure Class']).replace(/#/g, '').toUpperCase();
    const valveType = _rowText(row, ['valveType', 'Type Description', 'Valve Type', 'Type', 'Description']);
    if (rowBore === null || rowLength === null || rowWeight === null || !rowRating) continue;
    if (Math.abs(rowBore - boreMm) >= 1) continue;
    if (rowRating !== normalizedRating) continue;
    const lengthDelta = Math.abs(rowLength - lengthMm);
    if (lengthDelta > tolerance) continue;
    candidates.push({ weight: rowWeight, lengthDelta, valveType, rowData: row });
  }
  candidates.sort((a, b) => a.lengthDelta - b.lengthDelta);
  return candidates;
}

function _normalizedXmlCiiRating(value) {
  return _toText(value).replace(/#/g, '').trim().toUpperCase();
}

function _xmlCiiWeightCandidateFromRow(row, config, lengthMm) {
  const rowBore = _rowNumber(row, ['boreMm', 'convertedBore', 'Converted Bore', 'bore', 'Bore', 'DN', 'NB']);
  const rowLengthRaw = _rowNumber(row, ['lengthMm', 'length', 'Length', 'Length (RF-F/F)', 'RF-F/F', 'LEN', 'faceToFace']);
  const unit = _toText(config.weight?.masterLengthUnit || 'auto').toLowerCase();
  const rowLength = rowLengthRaw === null
    ...
    : (unit === 'in' || (unit === 'auto' && rowLengthRaw < 100 && lengthMm > 100)
      ...
      : rowLengthRaw);
  const rowWeight = _rowNumber(row, ['valveWeight', 'directWeight', 'weight', 'Weight', 'RF/RTJ KG', 'Valve Weight']);
  const rowRating = _normalizedXmlCiiRating(_rowText(row, ['ratingClass', 'rating', 'Rating', 'RATING', 'Class', 'CLASS', 'Pressure Class']));
  const valveType = _rowText(row, ['valveType', 'Type Description', 'Valve Type', 'Type', 'Description']);
  if (rowBore === null || rowLength === null || rowWeight === null) return null;
  return { weight: rowWeight, rowBore, rowLength, rowRating, valveType, rowData: row };
}

// Weighting for approximate weight matching: rating matters most (3x), then
// bore (2x), then length (1x).
const XML_CII_WEIGHT_MATCH_WEIGHTS = Object.freeze({ bore: 2, rating: 3, length: 1 });

function _scoreXmlCiiWeightCandidates({ boreMm, rating, lengthMm }, config) {
  const rows = Array.isArray(config.weight?.masterRows) ? config.weight.masterRows : [];
  if (!rows.length || boreMm === null || lengthMm === null) return [];
  const normalizedRating = _normalizedXmlCiiRating(rating);
  const lengthScale = Math.max(_toFiniteNumber(config.weight?.lengthToleranceMm, 4) * 5, Math.abs(lengthMm) * 0.2, 25);
  const boreScale = Math.max(Math.abs(boreMm) * 0.25, 25);
  const { bore: wBore, rating: wRating, length: wLength } = XML_CII_WEIGHT_MATCH_WEIGHTS;
  const wTotal = wBore + wRating + wLength;
  const candidates = [];
  for (const row of rows) {
    const base = _xmlCiiWeightCandidateFromRow(row, config, lengthMm);
    if (!base) continue;
    const lengthDelta = Math.abs(base.rowLength - lengthMm);
    const boreDelta = Math.abs(base.rowBore - boreMm);
    const lengthScore = 1 - Math.min(lengthDelta / lengthScale, 1);
    const boreScore = 1 - Math.min(boreDelta / boreScale, 1);
    let ratingScore = 0;
    if (base.rowRating && normalizedRating && base.rowRating === normalizedRating) ratingScore = 1;
    else if (base.rowRating && normalizedRating && (base.rowRating.includes(normalizedRating) || normalizedRating.includes(base.rowRating))) ratingScore = 0.65;
    const score = (boreScore * wBore + ratingScore * wRating + lengthScore * wLength) / wTotal;
    if (score <= 0) continue;
    // Preferred choice: exact bore + exact rating match. Linearly scale the
    // master weight to the actual length so a longer/shorter body of the same
    // bore and rating gets a proportionally heavier/lighter suggested weight.
    const preferred = boreDelta < 1 && ratingScore === 1;
    const suggestedWeight = (preferred && base.rowLength > 0)
      ? base.weight * (lengthMm / base.rowLength)
      : base.weight;
    candidates.push({
      ...base,
      lengthDelta,
  ...
  // Preferred (exact bore+rating) candidates rank first, then by score.
  candidates.sort((a, b) =>
    (Number(b.preferred) - Number(a.preferred))
    || (b.score - a.score)
    || (a.lengthDelta - b.lengthDelta)
    || (a.boreDelta - b.boreDelta));
  return candidates.slice(0, 5);
}

function _xmlCiiRigidWeightOverrideKey(branchName, nodeNumber) {
  return `${_toText(branchName).trim()}::${_toText(nodeNumber).trim()}`;
}

function _isXmlCiiRigidNode(node) {
  return _xmlText(node, 'ComponentType').toUpperCase() === 'RIGID' || Number(_xmlText(node, 'Rigid')) === 2;
}

function _xmlCiiNumberText(value) {
  const numeric = Number(_toText(value).replace(/,/g, '').trim());
...
function _xmlCiiAncestorBranchName(node) {
  ...
  while (current) {
    if (current.nodeType === 1 && _xmlLocalName(current) === 'Branch') return _xmlText(current, 'Branchname');
...
function _xmlCiiRigidWeightOverrideForNode(branchName, node, config) {
  ...
  const raw = overrides[_xmlCiiRigidWeightOverrideKey(branchName, nodeNumber)] ?? overrides[nodeNumber];
  const numeric = _xmlCiiNumberText(raw);
  return numeric !== null && numeric > 0 ? numeric : null;
}

function _xmlCiiForwardElementLengths(nodes) {
  // Length of the CII element that STARTS at each node: the distance to the next
  // node carrying a positive NodeNumber. The converter keeps only positive-
  // numbered nodes (unnumbered fittings such as GASK/FLAN with NodeNumber -1 are
  // collapsed), so a rigid/valve's true element length is this forward span — not
  // the backward distance to a co-located fitting (which is 0 and hides the
  // component from the 4A weight review).
  const out = new Array(nodes.length).fill(null);
  for (let i = 0; i < nodes.length; i += 1) {
    const fromPos = _xmlText(nodes[i], 'Position');
    if (!fromPos) continue;
    for (let j = i + 1; j < nodes.length; j += 1) {
      const num = Number(_xmlText(nodes[j], 'NodeNumber'));
      if (!Number.isFinite(num) || num <= 0) continue;
      const toPos = _xmlText(nodes[j], 'Position');
      if (toPos) out[i] = _pointDistanceMm(fromPos, toPos);
      break;
    }
  }
  return out;
...
function _collectXmlCiiZeroRigidWeightIssues(xmlText, stagedJsonText, config) {
  ...
  const stagedIndex = _buildStagedComponentIndex(stagedJsonText, config);
  const issues = [];
  for (const branch of [...document.getElementsByTagName('Branch')]) {
    ...
    branchNodeList.forEach((node, nodeIdx) => {
      const lengthRaw = _xmlText(node, 'ElementLengthMm');
      const lengthFromXml = lengthRaw ? _xmlCiiNumberText(lengthRaw) : null;
      const lengthMm = lengthFromXml !== null ? lengthFromXml : forwardLengths[nodeIdx];
      const weight = _xmlCiiNumberText(_xmlText(node, 'Weight'));
      const nodeNumber = _xmlText(node, 'NodeNumber');
      // Only positive-numbered nodes become CAESAR elements; -1 fittings are dropped.
      // Include every zero-weight rigid that starts a real element (forward length
      // defined, incl. 0 for genuine zero-length valves); skip dangling end-of-branch
      // occurrences so a branch-connection valve is listed once.
      if (_isXmlCiiRigidNode(node) && Number(nodeNumber) > 0 && lengthMm !== null && (weight === null || Math.abs(weight) <= 1e-12)) {
        const staged = _stagedComponentForXmlNode(node, stagedIndex, config);
        const boreMm = _xmlCiiNumberText(_xmlText(node, 'BoreMm')) ?? _xmlCiiNumberText(_xmlText(node, 'OutsideDiameter'));
        const rating = _xmlText(node, 'Rating');
        issues.push({
          key: _xmlCiiRigidWeightOverrideKey(branchName, nodeNumber),
          branchName,
          nodeNumber,
          nodeName: _xmlText(node, 'NodeName'),
          componentType: _xmlText(node, 'ComponentType'),
          boreMm,
          rating,
          lengthMm,
          dtxr: _stagedComponentDtxr(staged.match),
          stagedName: _toText(staged.match?.component?.name || staged.match?.attrs?.NAME || '').trim(),
          stagedMatchMethod: staged.method,
          candidates: _scoreXmlCiiWeightCandidates({ boreMm, rating, lengthMm }, config).slice(0, 5),
        });
      }
    });
  }
...
function _applyXmlRestraints(document, nodeElement, entries, config) {
  ...
  const existing = _xmlChildrenByName(nodeElement, 'Restraint');
  let stiffness = config.defaultStiffness || '1.751270E+12';
  let gap = '0';
  let friction = config.defaultFriction || '0.3';
  if (existing.length > 0) {
    const r0 = existing[0];
    const s = _xmlText(r0, 'Stiffness');
    if (s && Number.isFinite(Number(s)) && Number(s) > 0) stiffness = s;
    const g = _xmlText(r0, 'Gap');
    if (g) gap = g;
    const f = _xmlText(r0, 'Friction');
    if (f) friction = f;
  }
  for (const r of existing) nodeElement.removeChild(r);
  for (const entry of entries) {
    const type = _toText(typeof entry === 'string' ? entry : entry?.type).trim();
    if (!type) continue;
    const restraint = nodeElement?.namespaceURI
      ? document.createElementNS(nodeElement.namespaceURI, 'Restraint')
      : document.createElement('Restraint');
    nodeElement.appendChild(restraint);
    _xmlSetText(document, restraint, 'Type', type);
    _xmlSetText(document, restraint, 'Stiffness', _toText(entry?.stiffness).trim() || stiffness);
    _xmlSetText(document, restraint, 'Gap', _toText(entry?.gap).trim() || gap);
    _xmlSetText(document, restraint, 'Friction', _toText(entry?.friction).trim() || friction);
  }
}
...
async function _enrichXmlForCii2019(xmlText, stagedJsonText, options = {}) {
  ...
  await _loadXmlCiiMasterRows(config.pipingClass, ['../docs/Masters/Piping_class_master.json', 'docs/Masters/Piping_class_master.json'], 'piping-class', diagnostics);
  await _loadXmlCiiMaterialMap(config, diagnostics);
  const stagedSupportIndex = _buildStagedSupportIndex(stagedJsonText, config, diagnostics);
  const stagedComponentIndex = _buildStagedComponentIndex(stagedJsonText, config);
  const stagedDtxrPositionIndex = _buildStagedDtxrPositionIndex(stagedJsonText, config);
  const tolerance = _toFiniteNumber(config.coordinateTolerance, 1);
  ...
  for (const branch of [...document.getElementsByTagName('Branch')]) {
    ...
    // Customization: validate the derived class against the master (override ->
    // exact -> startsWith -> fuzzy), then fetch wall/corrosion/material by
    // class+bore, then resolve the material code (fuzzy + override).
    const classMatch = _xmlCiiApproximateClass(derivedClassRaw, config);
    const pipingClass = classMatch.pipingClass || derivedClassRaw;
    const pcMaster = _findPipingClassMaster({ pipingClass, boreMm: branchBore }, config);
    const branchRating = (pcMaster && pcMaster.rating) || _deriveRatingFromPipingClass(pipingClass, config) || _rowText(lineListMatch, ['rating', 'Rating']);
    const matResolved = pcMaster && pcMaster.materialName ? _xmlCiiResolveMaterialCode(pcMaster.materialName, config) : null;
    if (pcMaster || classMatch.method !== 'none') {
      diagnostics.push({ type: 'class-master-match', branchName, derivedClass: derivedClassRaw, pipingClass,
        classMethod: classMatch.method, classConfidence: classMatch.confidence,
        wallThickness: pcMaster?.wallThickness || '', corrosion: pcMaster?.corrosion || '',
        materialName: pcMaster?.materialName || '', materialCode: matResolved?.code || '',
        materialMethod: matResolved?.method || '', needsReview: !!(classMatch.needsReview || (matResolved && matResolved.needsReview)) });
    }
    ...
    branchNodeList.forEach((node, nodeIdx) => {
      ...
      if (pcMaster) {
        if (pcMaster.wallThickness) _xmlSetText(document, node, 'WallThickness', pcMaster.wallThickness);
        if (pcMaster.corrosion) _xmlSetText(document, node, 'CorrosionAllowance', pcMaster.corrosion);
        if (pcMaster.materialName) _xmlSetText(document, node, 'MaterialName', pcMaster.materialName);
        if (matResolved && matResolved.code) {
          _xmlSetText(document, node, 'MaterialCode', matResolved.code);
          // Also write to branch-level MaterialNumber (CII native field)
          _xmlSetText(document, branch, 'MaterialNumber', matResolved.code);
        }
  ...
  for (const node of [...document.getElementsByTagName('Node')]) {
    ...
    const tagMatches = _xmlNodeSupportTags(node).flatMap((tag) => stagedSupportIndex.byTag.get(tag) || []);
    ...
    const stagedMatches = tagMatches.length > 0 ? tagMatches : coordMatches;
    const staged = stagedMatches[0] || null;
    const restraints = _xmlChildrenByName(node, 'Restraint');
    const combinedTypes = Array.from(new Set(stagedMatches.flatMap(m => _xmlCiiTypeEntriesFromSupportKind(m.kind, config))));
    ...
    if (staged) {
      ...
      diagnostics.push({ type: 'support-match', nodeNumber: _xmlText(node, 'NodeNumber'), nodeName: _xmlText(node, 'NodeName'), method: tagMatches.length > 0 ? 'ps-tag' : 'coordinate-multi', kind: allKinds.join('+'), xmlRestraintTypes: xmlTypes, restraintTypes: combinedTypes.join('+'), stagedName: staged.component?.name || staged.attrs?.NAME || '', tags: _xmlNodeSupportTags(node) });
    }
...
export function renderModelConvertersTab(container) {
  ...
  container.innerHTML = `
    <div class="model-converters-root">
      <aside class="model-converters-left">
        ...
        <details id="model-converters-xml-cii-workflow" class="model-converters-workflow">
          <summary>XML->CII(2019) workflow</summary>
          <div class="model-converters-workflow-phase-list">
            ${XML_CII_WORKFLOW_PHASES.map((phase) => `
              <button type="button" class="model-converters-workflow-phase" data-xml-cii-phase="${_esc(phase.id)}">
                <span>${_esc(phase.label)}</span>
                <small>${_esc(phase.state)}</small>
              </button>
            `).join('')}
          </div>
  ...
  function xmlCiiRenderMasterRowsPanel(masterKey, config) {
    ...
    const previewRows = local.rawRows.length ? _xmlCiiMapRowsWithFieldMap(local.rawRows, fieldMap) : [];
      ...
      <div class="model-converters-workflow-master-card">
        ...
        <div class="model-converters-workflow-split">
          ...
          <section>
            ...
            ${_xmlCiiRowsToTableHtml(rowsForSavedPanel, 120)}
          </section>
        </div>
      </div>
    `;
  ...
  function xmlCiiRenderImportMastersPhase(rootEl) {
    ...
    const summary = XML_CII_MASTER_ORDER.map((masterKey) => {
      ...
      return {
        masterKey,
        rows: _xmlCiiMasterRowsFromConfig(config, masterDef).length,
      };
    });
  ...
  function xmlCiiRenderRunPhase() {
    ...
    return `
      ...
      <div style="margin-top:12px;">
        ...
        <div class="model-converters-workflow-detail-note" style="margin-top:6px;">Opens <strong>4A Weight Match</strong> to review approximate weights, then Finalize and Run.</div>
      </div>
    `;
  }
  ...
  function bindXmlCiiRunPhase(rootEl) {
    ...
    rootEl.querySelectorAll('[data-xml-cii-run-from-workflow]').forEach((btn) => {
      btn.addEventListener('click', () => {
        // Two-step run: review approximate weight matches in 4A first, then the
        // user clicks "Finalize and Run" there to launch the conversion.
  ...
  function xmlCiiRenderWeightMatchPhase() {
    return `
      <div class="model-converters-workflow-detail-title">4A Weight Match</div>
      <div class="model-converters-workflow-detail-text">
        Approximate component weights matched from the valve weight master by
        <strong>rating</strong> (×3), <strong>bore</strong> (×2), and <strong>length</strong> (×1).
        The preferred suggestion <strong>★</strong> has an exact bore + rating match with the weight
        linearly scaled to the actual length. Review and adjust, then <strong>Finalize and Run</strong>.
      </div>
      ...
      <div style="margin-top:14px;">
        <button type="button" class="model-converters-run-btn" id="mc-wm-finalize" style="width:100%;padding:12px;">
          ✓ Finalize and Run
        </button>
      </div>`;
  }
  ...
  function bindXmlCiiWeightMatchPhase(detailEl, rootEl) {
    ...
    const setStatus = (msg, tone) => { if (statusEl) { statusEl.textContent = msg || ''; statusEl.className = `mc-diag-run-status ${tone || ''}`.trim(); } };
    if (!contentEl) return;

    const renderIssues = (issues) => {
      if (!issues.length) {
        contentEl.innerHTML = '<div class="model-converters-workflow-detail-note">No rigid components need a weight decision — everything matched automatically (or there are no rigids). You can Finalize and Run.</div>';
        return;
      }
      const rowsHtml = issues.map((issue, index) => {
        ...
        const candidates = (issue.candidates || []).map((candidate) => {
          ...
          return `
            <button type="button" class="mc-rigid-review-candidate${candidate.preferred ? ' best' : ''}"
              data-wm-candidate="${index}" data-wm-weight="${_escAttr(value)}"
              title="${_escAttr(`${candidate.preferred ? 'Preferred (exact bore+rating, length-scaled) · ' : ''}Bore ${candidate.rowBore} | Rating ${candidate.rowRating || '-'} | Master length ${Number(candidate.rowLength).toFixed(1)} | Score ${Math.round(candidate.score * 100)}%`)}">
        ...
        return `
          <tr>
            ...
            <td><input type="number" min="0" step="0.001" class="mc-rigid-review-input" data-wm-key="${_escAttr(issue.key)}" value="${_escAttr(best ? (best.suggestedWeight ?? best.weight) : '')}" placeholder="kg"></td>
      ...
      contentEl.innerHTML = `
        <div class="mc-rigid-review-table-wrap" style="overflow:auto;max-height:48vh;">
          <table class="mc-rigid-review-table">
            <thead><tr><th>Branch</th><th>Bore</th><th>Rating</th><th>Node</th><th>DTXR</th><th>Length</th><th>Suggested Weight (kg)</th><th>Nearest Suggestions</th></tr></thead>
            <tbody>${rowsHtml}</tbody>
          </table>
        </div>`;
      contentEl.querySelectorAll('[data-wm-candidate]').forEach((button) => {
        button.addEventListener('click', () => {
          const rowIndex = Number(button.getAttribute('data-wm-candidate'));
          const input = contentEl.querySelectorAll('.mc-rigid-review-input')[rowIndex];
          if (input) input.value = button.getAttribute('data-wm-weight') || '';
        });
      });
    };
    ...
    const compute = async () => {
      ...
      try {
        ...
        const xmlText = _decodeTextUtf8(primaryBytes);
        const secondaryBytes = secondaryFile ? await secondaryFile.arrayBuffer() : null;
        const stagedJsonText = secondaryBytes ? _decodeTextUtf8(secondaryBytes) : (values.stagedAttributesJson || '');
        const config = _parseXmlCiiEnrichmentConfig(values.supportConfigJson);
        const enriched = await _enrichXmlForCii2019(xmlText, stagedJsonText, { ...values, supportConfigJson: JSON.stringify(config), dryRun: true });
        xmlCiiWeightMatchIssues = _collectXmlCiiZeroRigidWeightIssues(enriched.xmlText, stagedJsonText, enriched.config || config);
        setStatus(`${xmlCiiWeightMatchIssues.length} component(s) to review`, 'ok');
        renderIssues(xmlCiiWeightMatchIssues);
      } catch (err) {
        xmlCiiWeightMatchIssues = [];
        const message = _toText(err?.message || err);
        contentEl.innerHTML = `<div class="model-converters-workflow-detail-note">Could not compute weight matches: ${_esc(message)}</div>`;
        setStatus('Error', 'bad');
      }
    };
    ...
    detailEl.querySelector('#mc-wm-fill-best')?.addEventListener('click', () => {
      contentEl.querySelectorAll('.mc-rigid-review-input').forEach((input, index) => {
        const best = xmlCiiWeightMatchIssues[index]?.candidates?.[0];
        if (best) input.value = best.suggestedWeight ?? best.weight;
      });
    });
    detailEl.querySelector('#mc-wm-finalize')?.addEventListener('click', () => {
      // Persist reviewed weights as rigid weight overrides so the run applies
      // them (and skips the redundant mid-run review), then launch the run.
      const weightsByKey = {};
      contentEl.querySelectorAll('.mc-rigid-review-input').forEach((input) => {
        const key = input.getAttribute('data-wm-key') || '';
        const numeric = Number(input.value);
        if (key && Number.isFinite(numeric) && numeric > 0) weightsByKey[key] = numeric;
      });
      if (Object.keys(weightsByKey).length) {
    ...
    // Auto-compute on open so the review is ready immediately after clicking Run.
    void compute();
  }
  ...
  function xmlCiiRenderPreviewPhase(rootEl) {
    const xmlFile = primaryInputEl?.files?.[0];
    const config = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
    ...
    if (!xmlFile) {
      return `
        <div class="model-converters-workflow-detail-title">4 Preview</div>
        <div class="model-converters-workflow-detail-note">
          ⚠ Load an XML file in the sidebar first, then return here to preview enrichment.
        </div>`;
    }
    if (!llRows && !pcRows) {
      return `
        <div class="model-converters-workflow-detail-title">4 Preview</div>
        <div class="model-converters-workflow-detail-note">
          ⚠ Import at least one master in 2 Import Masters, then return here.
        </div>`;
    }
    // Return loading state; actual render happens async via bindXmlCiiPreviewPhase
    return `
      <div class="model-converters-workflow-detail-title">4 Preview</div>
      <div class="model-converters-workflow-detail-text">
        Dry-run enrichment per branch — editable overrides for approximate matches.
      </div>
      <div id="mc-preview-table-host">
        <div class="model-converters-workflow-detail-note" style="text-align:center;padding:18px;">
          Loading preview...
        </div>
      </div>`;
  }

  function _xmlCiiDryRunPreview(xmlText, config) {
    ...
    } catch { return { branchRows: [], nodeRows: [] }; }
    ...
    for (const branch of [...document.getElementsByTagName('Branch')]) {
      ...
      const lineListMatch = lineKey ? _xmlCiiFindLineListRow(lineKey, config) : null;
      const derivedClassRaw = _derivePipingClassFromBranchName(branchName, config) || _rowText(lineListMatch, ['pipingClass', 'PipingClass']);
      const boreMm = _deriveBoreFromBranchName(branchName, config) || _rowNumber(lineListMatch, ['convertedBore', 'Bore']);
      const classMatch = _xmlCiiApproximateClass(derivedClassRaw, config);
      const pipingClass = classMatch.pipingClass || derivedClassRaw;
      const pcMaster = _findPipingClassMaster({ pipingClass, boreMm }, config);
      const branchRating = (pcMaster?.rating) || _deriveRatingFromPipingClass(pipingClass, config) || _rowText(lineListMatch, ['rating', 'Rating']);
      const matResolved = (pcMaster?.materialName) ? _xmlCiiResolveMaterialCode(pcMaster.materialName, config) : null;

      // Merge processData overrides (user-typed P1/T1/T2/T3/Density from preview)
      const pdOverride = (lineKey && config?.overrides?.processData?.[lineKey]) || {};
      const p1 = _xmlCiiProcessValue(pdOverride, lineListMatch, 'p1', ['p1']);
      const t1 = _xmlCiiProcessValue(pdOverride, lineListMatch, 't1', ['t1']);
      const t2 = _xmlCiiProcessValue(pdOverride, lineListMatch, 't2', ['t2']);
      const t3 = _xmlCiiProcessValue(pdOverride, lineListMatch, 't3', ['t3']);
      const density = _xmlCiiProcessValue(pdOverride, lineListMatch, 'density', ['density']);
      const p1Source = Object.prototype.hasOwnProperty.call(pdOverride, 'p1') ? 'override' : (p1 ? 'linelist' : 'none');
      const t1Source = Object.prototype.hasOwnProperty.call(pdOverride, 't1') ? 'override' : (t1 ? 'linelist' : 'none');
      const t2Source = Object.prototype.hasOwnProperty.call(pdOverride, 't2') ? 'override' : (t2 ? 'linelist' : 'none');
      const t3Source = Object.prototype.hasOwnProperty.call(pdOverride, 't3') ? 'override' : (t3 ? 'linelist' : 'none');
      const densitySource = Object.prototype.hasOwnProperty.call(pdOverride, 'density') ? 'override' : (density ? 'linelist' : 'none');

      // Format wall thickness cleanly (no floating point noise)
      const wtRaw = pcMaster?.wallThickness || '';
      const wallThickness = wtRaw ? (isNaN(Number(wtRaw)) ? wtRaw : Number(Number(wtRaw).toPrecision(6)).toString()) : '';
      ...
      branchRows.push({
        ...
        pipingClassMethod: classMatch.method,
        pipingClassConfidence: classMatch.confidence,
        pipingClassNeedsReview: classMatch.needsReview,
        pipingClassCandidates: classMatch.candidates || [],
        material: pcMaster?.materialName || _rowText(lineListMatch, ['material', 'Material']) || '',
        materialCode: matResolved?.code || '',
        materialCodeMethod: matResolved?.method || 'none',
        materialCodeNeedsReview: !!(matResolved?.needsReview),
        rating: branchRating || '',
        p1, t1, t2, t3, density,
        p1Source, t1Source, t2Source, t3Source, densitySource,
        wallThickness,
        corrosion: pcMaster?.corrosion || '',
      });

      // Node-level weight candidates
      let previousPosition = null;
      for (const node of _xmlChildrenByName(branch, 'Node')) {
        const positionText = _xmlText(node, 'Position');
        const lengthMm = previousPosition ? _pointDistanceMm(previousPosition, positionText) : null;
        const candidates = (boreMm != null && branchRating && lengthMm != null)
          ? _findAllWeightCandidates({ boreMm, rating: branchRating, lengthMm }, config)
          : [];
  ...
  function _xmlCiiMatchBadgeHtml(method, confidence, needsReview, value, derivedValue) {
    if (!needsReview || method === 'exact' || method === 'override') {
      if (method === 'none' || !value) {
        return `<span class="mc-preview-badge bad" title="No match found">✗ no match</span>`;
      }
      return `<span class="mc-preview-exact">${_esc(value)}</span>`;
    }
    const pct = confidence != null ? ` (${Math.round(confidence * 100)}%)` : '';
    const label = method === 'startsWith' ? `≈ approx${pct}` : method === 'fuzzy' ? `≈ fuzzy${pct}` : method === 'ambiguous' ? '! ambiguous' : method === 'contains' ? `≈ contains` : `≈ ${method}${pct}`;
    const cls = method === 'ambiguous' ? 'orange' : 'amber';
    return `
      <span class="mc-preview-badge ${_esc(cls)}" title="${_esc(`Derived: ${derivedValue || value} | Matched: ${value} | Method: ${method}${pct}`)}">
        ${_esc(label)}
      </span>
      <span class="mc-preview-editable-val">${_esc(value || derivedValue || '')}</span>`;
  }

  function _mcProcessInputHtml(fieldKey, lineKey, currentVal, source, rowIndex) {
    const srcCls = source === 'override' ? 'mc-preview-pd-override' : source === 'linelist' ? 'mc-preview-pd-linelist' : 'mc-preview-pd-empty';
    const title = source === 'override' ? 'User override (will be used in conversion)' : source === 'linelist' ? 'From line list — editable' : 'Not in line list — type to set';
    return `<div class="mc-preview-pd-cell ${srcCls}">
      <input type="text" class="mc-preview-pd-input" value="${_esc(currentVal)}"
        data-mc-pd-field="${_esc(fieldKey)}" data-mc-pd-linekey="${_esc(lineKey)}" data-mc-pd-row="${rowIndex}"
        title="${_esc(title)}" placeholder="—">
      <button type="button" class="mc-preview-filldown-btn mc-pd-filldown"
        data-mc-fill-field="${_esc(fieldKey)}" data-mc-fill-from="${rowIndex}"
        title="Fill down same value to all rows below">↓</button>
    </div>`;
  }
  ...
  async function xmlCiiBuildAndRenderPreview(rootEl) {
    ...
    const config = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
    let xmlText;
    try { xmlText = await xmlFile.text(); } catch { host.innerHTML = '<div class="model-converters-workflow-detail-note">Failed to read XML file.</div>'; return; }
    const { branchRows, nodeRows } = _xmlCiiDryRunPreview(xmlText, config);

    if (!branchRows.length) {
      host.innerHTML = '<div class="model-converters-workflow-detail-note">No branches found in XML.</div>';
      return;
    }

    const nodesByBranch = {};
    for (const nr of nodeRows) {
      (nodesByBranch[nr.branchName] = nodesByBranch[nr.branchName] || []).push(nr);
    }

    // Column definitions: label, editable hint, alignment
    const colHdr = (label, editHint = '') => `<th class="mc-preview-th">${_esc(label)}${editHint ? `<span class="mc-preview-edit-hint">${editHint}</span>` : ''}</th>`;
    const COLS = [
      { label: 'Branch Name' }, { label: 'Line Key' }, { label: 'Size' },
      { label: 'Piping Class', hint: 'Edit' }, { label: 'Material' }, { label: 'Rating' },
      { label: 'Mat. Code', hint: 'Edit' },
      { label: 'P1 (MPa)', hint: 'Edit' }, { label: 'T1 (C)', hint: 'Edit' }, { label: 'T2 (C)', hint: 'Edit' }, { label: 'T3 (C)', hint: 'Edit' }, { label: 'Density', hint: 'Edit' },
      { label: 'Wall Thk' }, { label: 'Corrosion' }, { label: 'Wt' },
    ];
    const theadHtml = COLS.map((c) => colHdr(c.label, c.hint || '')).join('');

    host.innerHTML = `
      <div class="mc-preview-legend" id="mc-preview-legend">
        <span class="mc-preview-badge amber">≈ approx</span> Approximate match — click to override &nbsp;
        <span class="mc-preview-badge bad">✗</span> No match &nbsp;
        <span class="mc-preview-edit-hint">Edit</span> Editable &nbsp;
        <span class="mc-pd-src-dot override"></span> User override &nbsp;
        <span class="mc-pd-src-dot linelist"></span> From linelist &nbsp;
        <span class="mc-pd-src-dot empty"></span> Not mapped &nbsp;
        <button type="button" class="mc-preview-refresh-btn" id="mc-preview-refresh" title="Refresh preview from current config">Refresh</button>
      </div>
        ...
        <table class="mc-preview-table">
          ...
          <tbody>
            ${branchRows.map((row, ri) => {
              const nw = nodesByBranch[row.branchName] || [];
              const pcBadge = _xmlCiiMatchBadgeHtml(row.pipingClassMethod, row.pipingClassConfidence, row.pipingClassNeedsReview, row.pipingClass, row.pipingClassDerived);
              const mcBadge = row.material
                ? _xmlCiiMatchBadgeHtml(row.materialCodeMethod, null, row.materialCodeNeedsReview, row.materialCode, row.material)
                : '<span class="mc-preview-muted">—</span>';
              ...
              // Fill-down button always shown for approx cells
              const fillDownBtn = (field, fromRow) => `<button type="button" class="mc-preview-filldown-btn" data-mc-fill-field="${_esc(field)}" data-mc-fill-from="${fromRow}" title="Fill down same override to all rows below">↓</button>`;
              return `<tr class="mc-preview-row${row.lineMiss ? ' mc-preview-line-miss' : ''}">
                <td class="mc-preview-td mc-preview-branch" title="${_esc(row.branchName)}">${_esc(row.branchName.length > 36 ? '…' + row.branchName.slice(-32) : row.branchName)}</td>
                <td class="mc-preview-td${row.lineMiss ? ' mc-preview-warn' : ''}">${_esc(row.lineKey || '—')}</td>
                <td class="mc-preview-td">${_esc(row.size || '—')}</td>
                <td ${tdApprox(row.pipingClassNeedsReview, `data-mc-edit-type="pipingClass" data-mc-edit-derived="${_esc(row.pipingClassDerived)}" data-mc-edit-row="${ri}"`)}>
                  ${pcBadge}${row.pipingClassNeedsReview ? fillDownBtn('pipingClass', ri) : ''}
                </td>
                <td class="mc-preview-td">${_esc(row.material || '—')}</td>
                <td class="mc-preview-td">${_esc(row.rating || '—')}</td>
                <td ${tdApprox(row.materialCodeNeedsReview, `data-mc-edit-type="materialCode" data-mc-edit-mat="${_esc(row.material)}" data-mc-edit-row="${ri}"`)}>
                  ${mcBadge}${row.materialCodeNeedsReview ? fillDownBtn('materialCode', ri) : ''}
                </td>
                <td class="mc-preview-td mc-preview-pd-td">${_mcProcessInputHtml('p1', row.lineKey, row.p1, row.p1Source, ri)}</td>
                <td class="mc-preview-td mc-preview-pd-td">${_mcProcessInputHtml('t1', row.lineKey, row.t1, row.t1Source, ri)}</td>
                <td class="mc-preview-td mc-preview-pd-td">${_mcProcessInputHtml('t2', row.lineKey, row.t2, row.t2Source, ri)}</td>
                <td class="mc-preview-td mc-preview-pd-td">${_mcProcessInputHtml('t3', row.lineKey, row.t3, row.t3Source, ri)}</td>
                <td class="mc-preview-td mc-preview-pd-td">${_mcProcessInputHtml('density', row.lineKey, row.density, row.densitySource, ri)}</td>
                <td class="mc-preview-td">${_esc(row.wallThickness || '—')}</td>
                <td class="mc-preview-td">${_esc(row.corrosion || '—')}</td>
                <td class="mc-preview-td">${wIcon}</td>
              </tr>
              <tr class="mc-preview-node-row" id="mc-preview-nodes-${ri}" style="display:none">
                <td colspan="${COLS.length}" class="mc-preview-node-cell">
                  ${nw.length ? `<table class="mc-preview-node-table">
                    <thead><tr><th>Node</th><th>Type</th><th>Bore</th><th>Rating</th><th>Length</th><th>Weight</th><th>ΔLen</th><th>All Candidates (Valve Type · Weight · ΔLength)</th></tr></thead>
                    <tbody>${nw.map((nr) => `<tr>
                      <td>${_esc(nr.nodeNumber)}</td>
                      <td>${_esc(nr.componentType)}</td>
                      <td>${nr.boreMm != null ? nr.boreMm.toFixed(0) + 'mm' : '—'}</td>
                      <td>${_esc(nr.rating)}</td>
                      <td>${nr.lengthMm != null ? nr.lengthMm.toFixed(1) + 'mm' : '—'}</td>
                      <td>${nr.weightMatch ? nr.weightMatch.weight + ' kg' : '—'}</td>
                      <td>${nr.weightMatch ? nr.weightMatch.lengthDelta.toFixed(1) + 'mm' : '—'}</td>
                      <td class="mc-preview-candidates">${nr.weightCandidates.map((c, ci) =>
                        `<span class="mc-preview-candidate${ci === 0 ? ' best' : ''}" title="${_esc(c.valveType)}">${_esc(c.valveType || 'Unknown')} · ${c.weight}kg · Δ${c.lengthDelta.toFixed(1)}mm</span>`
                      ).join('')}</td>
                    </tr>`).join('')}</tbody>
                  </table>` : ''}
                </td>
    ...
    // ---- Refresh button ----
    host.querySelector('#mc-preview-refresh')?.addEventListener('click', () => {
      xmlCiiBuildAndRenderPreview(rootEl).catch(() => {});
    });

    // ---- Weight toggle buttons ----
    host.querySelectorAll('[data-mc-preview-row]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const ri = btn.getAttribute('data-mc-preview-row');
        const nodeRow = host.querySelector(`#mc-preview-nodes-${ri}`);
        if (nodeRow) nodeRow.style.display = nodeRow.style.display === 'none' ? '' : 'none';
      });
    });

    // ---- Editable piping class / material code cells (popup override) ----
    host.querySelectorAll('[data-mc-edit-type]').forEach((td) => {
      td.style.cursor = 'pointer';
      td.addEventListener('click', (e) => {
        if (e.target.closest('.mc-preview-filldown-btn')) return;
        const editType = td.getAttribute('data-mc-edit-type');
        const derivedKey = td.getAttribute('data-mc-edit-derived') || td.getAttribute('data-mc-edit-mat') || '';
        const currentVal = td.querySelector('.mc-preview-editable-val')?.textContent?.trim() || '';
        _xmlCiiOpenPreviewOverridePopup({ editType, derivedKey, currentVal, config, onSave: (newVal) => {
          const cfg = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
          const overrides = xmlCiiEnsureOverrides(cfg);
          if (editType === 'pipingClass') overrides.pipingClass = { ...overrides.pipingClass, [derivedKey]: newVal };
          else if (editType === 'materialCode') overrides.material = { ...overrides.material, [derivedKey]: newVal };
          xmlCiiSaveConfig(cfg);
          // Update badge text in-place — no full re-render
          const valSpan = td.querySelector('.mc-preview-editable-val');
          if (valSpan) valSpan.textContent = newVal;
          const badge = td.querySelector('.mc-preview-badge');
          if (badge) { badge.textContent = '✓ override'; badge.className = 'mc-preview-badge exact'; }
        }});
      });
    });

    // ---- Fill-down for piping class / material code (in-place, no re-render) ----
    host.querySelectorAll('.mc-preview-filldown-btn[data-mc-fill-field]:not(.mc-pd-filldown)').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const field = btn.getAttribute('data-mc-fill-field') || '';
        const fromRow = Number(btn.getAttribute('data-mc-fill-from') || 0);
        const sourceTd = btn.closest('[data-mc-edit-type]');
        const currentVal = sourceTd?.querySelector('.mc-preview-editable-val')?.textContent?.trim() || '';
        if (!currentVal) return;
        const cfg = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
        const overrides = xmlCiiEnsureOverrides(cfg);
        let filled = 0;
        host.querySelectorAll(`[data-mc-edit-type="${field}"]`).forEach((td) => {
          const tdRow = Number(td.getAttribute('data-mc-edit-row') || 0);
          if (tdRow <= fromRow) return;
          const tdKey = td.getAttribute('data-mc-edit-derived') || td.getAttribute('data-mc-edit-mat') || '';
          if (field === 'pipingClass') overrides.pipingClass = { ...overrides.pipingClass, [tdKey]: currentVal };
          else if (field === 'materialCode') overrides.material = { ...overrides.material, [tdKey]: currentVal };
          // Update DOM in-place
          const valSpan = td.querySelector('.mc-preview-editable-val');
        ...
        if (filled > 0) xmlCiiSaveConfig(cfg);
        // Flash confirmation on the button
        btn.textContent = `↓ ${filled}`;
        btn.style.color = '#5df0a0';
        setTimeout(() => { btn.textContent = '↓'; btn.style.color = ''; }, 1800);
      });
    });

    // ---- P1/T1/T2/T3/Density inline inputs — save only, no re-render ----
    host.querySelectorAll('.mc-preview-pd-input').forEach((input) => {
      const saveInput = () => {
        const field = input.getAttribute('data-mc-pd-field') || '';
        const lineKey = input.getAttribute('data-mc-pd-linekey') || '';
        if (!lineKey) return;
        const val = input.value; // do NOT trim here so cursor position is preserved
        const cfg = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
        const overrides = xmlCiiEnsureOverrides(cfg);
        if (!overrides.processData) overrides.processData = {};
        overrides.processData[lineKey] = { ...(overrides.processData[lineKey] || {}), [field]: val.trim() };
        xmlCiiSaveConfig(cfg);
        // Update cell's source indicator in-place
        const cell = input.closest('.mc-preview-pd-cell');
        if (cell) {
          cell.classList.remove('mc-preview-pd-empty', 'mc-preview-pd-linelist', 'mc-preview-pd-override');
          cell.classList.add(val.trim() ? 'mc-preview-pd-override' : 'mc-preview-pd-empty');
        }
      };
      input.addEventListener('input', saveInput);   // save on every keystroke (Bug 2b)
      input.addEventListener('change', saveInput);
      input.addEventListener('blur', saveInput);
      input.addEventListener('click', (e) => e.stopPropagation());
    });

    // ---- P1/T1/T2/T3/Density fill-down (in-place, no re-render) ----
    host.querySelectorAll('.mc-pd-filldown').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const field = btn.getAttribute('data-mc-fill-field') || '';
        const fromRow = Number(btn.getAttribute('data-mc-fill-from') || 0);
        const sourceInput = btn.previousElementSibling;
        const val = sourceInput?.value?.trim() || '';
        if (!val) {
          // Hint user to type a value first
          sourceInput?.focus();
          sourceInput?.select();
          return;
        }
        ...
        if (!overrides.processData) overrides.processData = {};
        let filled = 0;
        // Update all matching inputs in-place
        host.querySelectorAll('.mc-preview-pd-input').forEach((inp) => {
          if (inp.getAttribute('data-mc-pd-field') !== field) return;
          const rowIdx = Number(inp.getAttribute('data-mc-pd-row') || 0);
          if (rowIdx <= fromRow) return;
          const lk = inp.getAttribute('data-mc-pd-linekey') || '';
          if (!lk) return;
          overrides.processData[lk] = { ...(overrides.processData[lk] || {}), [field]: val };
          // Update DOM value in-place
          inp.value = val;
          const cell = inp.closest('.mc-preview-pd-cell');
          if (cell) {
            cell.classList.remove('mc-preview-pd-empty', 'mc-preview-pd-linelist');
            cell.classList.add('mc-preview-pd-override');
          }
          filled++;
        });
        if (filled > 0) xmlCiiSaveConfig(cfg);
  ...
  function xmlCiiSaveMasterRows(masterKey) {
    const masterDef = XML_CII_MASTER_DEFS[masterKey];
    const local = xmlCiiLocalMasterState(masterKey);
    const rows = _xmlCiiMapRowsWithFieldMap(local.rawRows, local.fieldMap);
  ...
  function bindXmlCiiImportMastersPhase(rootEl) {
    ...
    rootEl.querySelectorAll('[data-xml-cii-save-master]').forEach((button) => {
      button.addEventListener('click', () => xmlCiiSaveMasterRows(button.getAttribute('data-xml-cii-save-master') || 'linelist'));
    });
  ...
  function bindXmlCiiRegexFields(rootEl) {
    if (!rootEl) return;
    const detailEl = xmlCiiWorkflowDetailForRoot(rootEl);
    rootEl.querySelectorAll('[data-xml-cii-regex-path]').forEach((input) => {
      const saveRegexValue = () => {
        ...
        const value = input.type === 'number'
          ...
          : input.value;
        _setValueAtPath(config, path, value);
        xmlCiiSaveConfig(config);
      };
      input.addEventListener('input', saveRegexValue);
  ...
  function xmlCiiRenderWorkflowRoot(rootEl, detailEl) {
    ...
    if (selectedPhase.id === 'regex') bindXmlCiiRegexFields(rootEl);
  ...
  function bindXmlCiiDiagnosticsPhase(detailEl, rootEl) {
    ...
    detailEl?.querySelector('#mc-diag-dry-run-btn')?.addEventListener('click', async () => {
      ...
      try {
        ...
        const config = _parseXmlCiiEnrichmentConfig(values.supportConfigJson);
        const diagnostics = [];
        const enriched = await _enrichXmlForCii2019(xmlText, stagedJsonText, { ...values, supportConfigJson: JSON.stringify(config), dryRun: true, _diagOut: diagnostics });
  ...
  function xmlCiiRenderSupportMapperPhase() {
    return `
      <div class="model-converters-workflow-detail-title">6 Support Types</div>
      <div class="model-converters-workflow-detail-text">Map ATT/RVM fields to CII support kinds. Fields and keywords accept comma-separated values; use <code>*</code> to scan all attributes.</div>
      <div id="mc-support-mapper-host" style="overflow:auto;"></div>`;
  }
  ...
  function bindXmlCiiSupportMapperPhase(detailEl) {
    ...
    if (!host) return;
    try {
      renderSupportMapperPanel(host);
    } catch (e) {
      host.innerHTML = `<div class="model-converters-workflow-detail-note">Support mapper unavailable: ${_esc(e?.message || String(e))}</div>`;
    }
  }
  ...
  function bindXmlCiiConfigPhase(detailEl) {
    ...
    const saveNow = () => {
      ...
      try {
        ...
        xmlCiiSaveConfig(JSON.parse(merged));
    ...
    detailEl?.querySelector('#mc-cfg-import-file')?.addEventListener('change', async (e) => {
      ...
      try {
        const text = await file.text();
        xmlCiiSaveConfig(JSON.parse(text)); // validate + save full config
        if (textarea) textarea.value = _xmlCiiCompactConfigForEditor(activeValues().supportConfigJson || '').text;
  ...
  function xmlCiiCloseWorkflowPopup() {
    if (!xmlCiiWorkflowPopupEl) return;
  ...
  function xmlCiiOpenWorkflowPopup() {
    if (xmlCiiWorkflowPopupEl) {
      xmlCiiRenderWorkflowRoot(xmlCiiWorkflowPopupEl, xmlCiiWorkflowPopupDetailEl);
      return;
    }
    const overlay = document.createElement('div');
    overlay.className = 'model-converters-workflow-popup-overlay';
    overlay.dataset.selectedPhase = xmlCiiWorkflowEl?.dataset.selectedPhase || 'regex';
    overlay.dataset.activeMaster = xmlCiiWorkflowEl?.dataset.activeMaster || 'linelist';
    overlay.innerHTML = `
      <div class="model-converters-workflow-popup" role="dialog" aria-modal="true" aria-label="XML to CII workflow">
        <div class="model-converters-workflow-popup-head">
          <div>
            <div class="model-converters-workflow-popup-title">XML-&gt;CII(2019) Workflow</div>
            <div class="model-converters-workflow-detail-text">Import masters, map fields, and maintain manual overrides in a standalone-ready config.</div>
          </div>
          <button type="button" class="model-converters-download-btn model-converters-workflow-popup-close" data-xml-cii-popup-close>Close</button>
        </div>
        ...
        <div class="model-converters-workflow-popup-body" data-xml-cii-workflow-detail></div>
      </div>
    `;
    document.body.appendChild(overlay);
    xmlCiiWorkflowPopupEl = overlay;
    xmlCiiWorkflowPopupDetailEl = overlay.querySelector('[data-xml-cii-workflow-detail]');
    overlay.querySelector('[data-xml-cii-popup-close]')?.addEventListener('click', xmlCiiCloseWorkflowPopup);
    overlay.addEventListener('click', (event) => {
      if (event.target === overlay) xmlCiiCloseWorkflowPopup();
    });
    xmlCiiRenderWorkflowRoot(overlay, xmlCiiWorkflowPopupDetailEl);
    void xmlCiiEnsureDefaultMastersLoaded(overlay);
  }

  function renderXmlCiiWorkflowShell() {
    if (!xmlCiiWorkflowEl || !xmlCiiWorkflowDetailEl) return;
    const isXmlCiiConverter = activeDef().id === 'xml_to_cii';
    xmlCiiWorkflowEl.hidden = !isXmlCiiConverter;
    ...
    xmlCiiWorkflowDetailEl.innerHTML = `
      <div class="model-converters-workflow-detail-title">Open Master Workflow</div>
      <div class="model-converters-workflow-detail-text">
        Click the XML-&gt;CII(2019) workflow header to load master tabs and manual overrides in a popup window.
      </div>
    `;

    if (xmlCiiWorkflowEl.dataset.bound === 'true') return;
    xmlCiiWorkflowEl.querySelector('summary')?.addEventListener('click', (event) => {
      event.preventDefault();
      xmlCiiWorkflowEl.open = false;
    ...
    try {
      ...
      if (selectedConverter === 'rvm_to_rev') {
        ...
        if (primaryFile && primaryBytes) {
          ...
          const attText = _decodeTextUtf8(secondaryBytes);
          response = _buildRvmAttrResponseFromAttText(attText, secondaryFile.name, runValues);
        } else {
      ...
      } else if (selectedConverter === 'xml_to_cii' && runValues.createEnrichedXml) {
        if (!primaryFile || !primaryBytes) {
          throw new Error('Primary XML input is required for XML->CII(2019).');
        }
        const originalXmlText = _decodeTextUtf8(primaryBytes);
        const stagedJsonText = secondaryBytes ? _decodeTextUtf8(secondaryBytes) : '';
        const enriched = await _enrichXmlForCii2019(originalXmlText, stagedJsonText, runValues);
        const rigidReviewLogLines = [];
        const rigidWeightIssues = _collectXmlCiiZeroRigidWeightIssues(enriched.xmlText, stagedJsonText, enriched.config);
        if (rigidWeightIssues.length > 0) {
          setStatus(`Review needed: ${rigidWeightIssues.length} rigid weight(s) are zero.`, 'running');
          const review = await _openXmlCiiZeroRigidWeightPopup(rigidWeightIssues);
          if (review.cancelled) {
            const message = 'Cancelled: rigid zero-weight review was dismissed.';
            setStatus(message, '');
            outputEl.innerHTML = '<span class="model-converters-muted">No output generated.</span>';
            setLogs([message]);
            _3DModelConv_resetPreview('Preview not available yet.');
          ...
          if (review.skipped) {
            ...
            const cfg = _parseXmlCiiEnrichmentConfig(activeValues().supportConfigJson);
            const overrides = xmlCiiEnsureOverrides(cfg);
            overrides.rigidWeight = { ...(overrides.rigidWeight || {}), ...(review.weightsByKey || {}) };
            xmlCiiSaveConfig(cfg);
            const applied = _applyXmlCiiRigidWeightOverrides(enriched.xmlText, review.weightsByKey);
            ...
            enriched.diagnosticRows = _diagnosticRowsForTable(enriched.diagnostics);
            enriched.diagnosticText = JSON.stringify({ generatedAt: new Date().toISOString(), stats: enriched.stats, diagnostics: enriched.diagnostics }, null, 2);
            rigidReviewLogLines.push(`Rigid zero-weight review applied: ${applied.appliedCount} manual rigid weight(s).`);
          }
        }
        const enrichedName = `${_baseNameWithoutExtension(primaryFile.name)}_enriched.xml`;
        const ciiResponse = await (await ensureRuntime()).runJob({
          converterId: selectedConverter,
          inputFiles: [{ role: 'primary', name: enrichedName, bytes: _encodeTextUtf8(enriched.xmlText) }],
          options: { ...runValues, createEnrichedXml: false },
        });
        ...
        response = {
          outputs: [
            ...
            { name: `${_baseNameWithoutExtension(primaryFile.name)}_enrichment_diagnostics.json`, text: enriched.diagnosticText, mime: 'application/json;charset=utf-8' },
            ...ciiOutputs,
          ],
          logs: {
            stdout: [
              'Created enriched XML before XML->CII(2019).',
              `DATUM duplicate support nodes removed: ${enriched.stats.removedDuplicateSupports}.`,
              `XML restraints normalized: ${enriched.stats.normalizedRestraints}.`,
              `Staged JSON support matches applied: ${enriched.stats.stagedSupportsMapped}.`,
              `DTXR_PS annotations: ${enriched.stats.dtxrPsAnnotations || 0}.`,
              `DTXR_POS annotations: ${enriched.stats.dtxrPosAnnotations || 0}.`,
              `Branch line keys annotated from Branchname: ${enriched.stats.branchLineKeys}.`,
              `Rating annotations: ${enriched.stats.ratingAnnotations}; weight annotations: ${enriched.stats.weightAnnotations}.`,
              ...rigidReviewLogLines,
              `Enrichment diagnostics written: ${_baseNameWithoutExtension(primaryFile.name)}_enrichment_diagnostics.json`,
              ...(ciiResponse.logs?.stdout || []),
            ],
            stderr: [...(ciiResponse.logs?.stderr || [])],
          },
          diagnosticsRows: enriched.diagnosticRows,
        };
      } else {
        if (!primaryFile || !primaryBytes) {
