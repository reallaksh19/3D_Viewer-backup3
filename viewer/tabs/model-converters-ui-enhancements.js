...
function _syncMasterCardCounts(root, config) { const counts = { 'Piping Class': config?.pipingClass?.masterRows?.length || 0, 'Material Map': config?.material?.mapRows?.length || 0, 'Weights / Valve CA8': config?.weight?.masterRows?.length || 0 }; for (const card of root.querySelectorAll('.model-converters-workflow-master-card')) { const text = card.textContent || ''; const entry = Object.entries(counts).find(([label]) => text.includes(label)); if (!entry) continue; const strong = card.querySelector('strong'); if (strong) strong.textContent = `${entry[1]} row(s)`; } }
